...
def Homographie(idr,idc,idr2,idc2,colored_image,colored_image2):
    ...
    #nbx = np.maximum(np.abs(np.max(idr)),np.abs(row-np.min(idr)))
    #nby = np.maximum(np.abs(np.max(idc)),np.abs(col-np.min(idc)))
    ...
    print("H :\n",H)
    print("H_inv :\n",T_inv)
    img_nn = np.empty((img_T.shape[0], img_T.shape[1], 4), dtype=np.uint8)
    for i, row in enumerate(img_T):
        for j, col in enumerate(row):
            img_nn[i, j, :] = nearest_neighbors(i+minX_1, j+minY_1, colored_image, T_inv,h,minX,minY)

    for i, row in enumerate(gray_image2):
        for j, col in enumerate(row):
            img_nn[i-minX_1, j-minY_1, :] = colored_image2[i][j]
